<script lang="ts">
    ...
    // STEPS Logic (Refactored to match Narrative Synthesis Wizard UI)
    let currentStep = $state(0);
    const TOTAL_STEPS = 4; // 1: Core, 2: Structure DNA, 3: Characters, 4: Beat Sheet
    ...
    // Critical Effect Logic (consistent with CriticDisplay)
    function isCritical(val: number): boolean {
        return val <= 20; // Critical or Poor threshold
    }
...
<div class="detailed-wizard-container win95-window">
    <div class="win95-titlebar">
        <div class="win95-titlebar-text">Narrative Synthesis Wizard - Architect Mode</div>
    </div>
    
    <div class="wizard-body">
        <!-- LEFT SIDEBAR IMAGE (Matches Simple Wizard) -->
        <div class="wizard-sidebar">
            <div class="sidebar-image-placeholder">
                <div class="sidebar-computer-icon">🧙‍♂️</div>
                <div style="color: #fff; font-weight: bold; margin-top: 10px; font-size: 10px;">STEP {currentStep + 1} / {TOTAL_STEPS}</div>
            </div>
        </div>
    ...
    <div class="wizard-footer">
        ...
        <div class="footer-buttons">
            ...
            <button class="win95-btn" onclick={nextStep} disabled={currentStep === TOTAL_STEPS - 1}>Next &gt;</button>
        </div>
    </div>
</div>

<style>
    /* New Layout Styles mimicking WizardSimple.svelte */
    .detailed-wizard-container {
        display: flex;
        flex-direction: column;
        height: 600px; /* Fixed height for Wizard feel */
